...
class Petugas extends CI_Controller {
    ...
    function buku_edit($id) {
        ...
        $data['buku'] = $this->M_data->get_data('m_data_buku', $where)->result();
        $this->load->view('petugas/v_header');
    ...
    function buku_hapus($id) {
        $where = array('iddata_buku' => $id);
        $this->M_data->delete_data('m_data_buku', $where);
        redirect(base_url() . 'petugas/buku');
    }
    var $API ="";
    function ambilsiak(){
    ...
    function api_input() {
        ...
        $data['databiodata'] = json_decode($this->curl->simple_get($this->API.'/biodata'));
        $this->db->trans_begin();
        foreach ($data['databiodata'] as $row) {
            $insert_data = array(
                "NPM_mhs" => $row->id,
                "biodata_mhs_nama" => $row->nama
            );
            $this->M_data->insert_data($insert_data, 'm_biodata_mhs');
        }
    ...
    function hapus_anggota() {
        ...
        redirect(base_url().'petugas/hapus');
    }
    ...
    function petugas() {
        ...
        $this->load->view('petugas/v_header');
        $this->load->view('petugas/v_konsultasi', $data);
        $this->load->view('petugas/v_footer');
    }
    public function gejala() {
        $data['gejala'] = $this->M_data->get_data('gejala')->result();
        $this->load->view('petugas/v_header');
        $this->load->view('petugas/v_gejala', $data);
        $this->load->view('petugas/v_footer');
    }
